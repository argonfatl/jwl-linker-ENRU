...
Если хочешь, я могу сделать вторую версию документа:

- **таблица “метод → входы → выходы → побочные эффекты (сеть/редактор/история)”**
- или **референс 1:1 по всем методам класса `JWLLinkerPlugin` в порядке следования в файле** (получится очень большой документ).
